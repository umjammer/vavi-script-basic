...
{	KeyboardBuffer window;
	windowInputStream (KeyboardBuffer window) {this.window = window;}
	public int available() {window.outputstream.safeflush(); return(window.available()); }
	public int read() { window.outputstream.safeflush(); return(window.read()); }
...
{ private Thread suspendedthread;		// the guy we zapped
  ...
  private boolean linemode = true;	// true if we don't admit input is there
	...
	/* constructors */
	public KeyboardBuffer(String name) { super(name); }
	public KeyboardBuffer(int rows,int columns) { super(rows,columns); }
	public KeyboardBuffer(String name,int rows, int cols) { super(name,rows,cols); }
	
	/* get the raw input and output streams */
	OutputStream OutputStream() { return(outputstream);}
	InputStream InputStream() { return(inputstream);}
	/* get the formatted input and output streams */
	PrintStream PrintStream() { return (new PrintStream(outputstream,true)); }
